...
class SemaphoreWrapper:
    ...
    def __init__(self, semaphore: Semaphore):
        ...
        self.count = semaphore.count
        self.queue = [node for (node, _) in semaphore.queue]

    def __eq__(self, other):
        return self.id == other.id and self.count == other.count and self.queue == other.queue

    def __hash__(self):
        return self.id + self.count + sum([operation.__hash__() for operation in self.queue])
...
class Execution:
    ...
    def copy(self)-> 'Execution':
        ...
        for sem in list(self.semaphores.values()):
            semaphores[sem.id] = Semaphore(sem.id, sem.count, sem.original_id)   
        res = Execution(self.semaphores, self.execution_chain)
        for key in list(self.semaphores.keys()):
            res.semaphores[key].queue = copy.copy(self.semaphores[key].queue)
...
class State:
    ...
    def __init__(self, execution: Execution):
        ...
        self.semaphores = tuple(sorted([SemaphoreWrapper(semaphore) for semaphore in execution.semaphores.values()]))
...
class Graph:
    ...
    # Returns true if there is no deadlock.
    def check_all_possible_executions(self, res: List['Node'], nodes: List['Node'], execution: Execution,
                                      executed: bool) -> tuple[
        bool, Union[Execution, None]]:

        if State(execution) in self.safe_states:
            return True, None

        flag = False

        for node in list(nodes):
            nodes.extend(node.next)
            ...
            result, final_execution = self.check_all_possible_executions(res, nodes, new_execution, new_executed)

            if result:
                self.safe_states.add(State(new_execution))
            else:
    ...
    def is_deadlock_free(self) -> tuple[bool, Union[None, Execution]]:
        ...
        async_results = []
        for i, j, k in itertools.product(list(self.semaphores.values()), repeat=3):
            if k.id >= j.id >= i.id:
                ...
                async_results.append(pool.apply_async(Graph.check_all_possible_executions, [simplified_graph, [], [simplified_graph.root],
                                                                                empty_exec, True]))
