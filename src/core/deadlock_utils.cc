...
using dumped_value = std::map<const char*, size_t>;

namespace seastar::internal::deadlock_detection {
...
void trace_runtime_semaphore_constructor(const void* sem, size_t count) {
...
void trace_runtime_semaphore_destructor(const void* sem, size_t count) {
    auto serialized_sem = serialize_semaphore(sem, count);
...
void trace_runtime_semaphore_signal_caller(const void* sem, size_t count, runtime_vertex caller) {
...
void trace_runtime_semaphore_signal_schedule(const void* sem, runtime_vertex unlocked) {
    std::map<const char*, dumped_value> data{{
...
void trace_runtime_semaphore_wait(const void* sem, runtime_vertex caller) {
    std::map<const char*, dumped_value> data{{
            "semaphore_wait",
            {{"address", reinterpret_cast<uintptr_t>(sem)},
                    {"caller", caller.get_ptr()}}
