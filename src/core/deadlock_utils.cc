...
struct json_exported : public seastar::json::jsonable {
    std::string _string;

    explicit json_exported(std::string string) : _string(std::move(string)) {}
...
using dumped_type = std::variant<std::string, uintptr_t, json_exported, nullptr_t>;
...
struct json_object : public seastar::json::jsonable {
    ...
    json_object(json_exported value) : _value(value) {}
    std::string to_json() const override {
        if (auto ptr = std::get_if<std::string>(&_value)) {
            return "\"" + *ptr + "\"";
        }
        if (auto ptr = std::get_if<json_exported>(&_value)) {
            return ptr->to_json();
        }
...
static dumped_value serialize_vertex(runtime_vertex v) {
    return {{"value", v._ptr},
            ...
            {"type", v._type->name()},
            {"info", json_exported(v._extra_json)}};
}
