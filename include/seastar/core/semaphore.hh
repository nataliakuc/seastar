...
public:
    ...
    basic_semaphore(size_t count) : _count(count) {
        internal::trace_runtime_semaphore_constructor(this);
    }
    ...
    future<> wait(time_point timeout, size_t nr = 1) noexcept {
        ...
        internal::trace_runtime_semaphore_wait(this, &e.pr);
        try {
    ...
    void signal(size_t nr = 1) {
        internal::trace_runtime_semaphore_signal_caller(this, nr, internal::get_current_traced_ptr());
        if (_ex) {
            return;
        }
        _count += nr;
        while (!_wait_list.empty() && has_available_units(_wait_list.front().nr)) {
            ...
            _count -= x.nr;
            internal::trace_runtime_semaphore_signal_schedule(this, &x.pr);
            x.pr.set_value();
